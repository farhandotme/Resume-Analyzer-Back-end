...
// ─── Loading messages cycled during analysis ─────────────────────────────────
      ...
      <div style={S.card}>
        ...
        <div style={S.header}>
          ...
          <div>
            ...
            <p style={S.subtitle}>Upload your PDF · Get instant ATS feedback</p>
          </div>
        </div>
              ...
              <div style={S.sizeBarBg}>
                <div
                  style={{
                    ...
                    background:
                      file.size > MAX_SIZE_BYTES * 0.8 ? "#f97316" : "#6366f1",
                  }}
                />
              </div>
            </div>
            <button style={S.removeBtn} onClick={handleRemove} title="Remove">
              <svg
                width="15"
                height="15"
                viewBox="0 0 24 24"
        ...
        {analyzing && (
          <div style={S.analyzingCard}>
            <div style={S.analyzingTop}>
              ...
              <span style={S.analyzingTitle}>Analyzing your resume</span>
            </div>
            <p style={S.analyzingMsg}>{LOADING_MESSAGES[loadingMsgIndex]}</p>
              ...
              {LOADING_MESSAGES.map((msg, i) => (
                <div key={i} style={S.analyzingStep}>
                  <div
                    style={{
                      ...S.stepDot,
                      background: i <= loadingMsgIndex ? "#6366f1" : "#2a2a35",
                      boxShadow:
                        i === loadingMsgIndex ? "0 0 8px #6366f1" : "none",
                    }}
                  />
                  <span
                    style={{
                      ...S.stepLabel,
                      color: i <= loadingMsgIndex ? "#a5b4fc" : "#334155",
                    }}
                  >
                    {msg}
                  </span>
        ...
        {analysisResult && !analyzing && (
          <div style={S.resultWrap}>
              ...
              <div style={S.heroRight}>
                <div style={S.heroRow}>
                  ...
                  <span style={S.hireBadge}>
                    {analysisResult.hero.hire_probability} hire probability
                  </span>
                </div>
                <p style={S.heroOneLiner}>{analysisResult.hero.one_liner}</p>
              </div>
            </div>
                ...
                return (
                  <div key={item.label} style={S.breakdownRow}>
                    <div style={S.breakdownHead}>
                      ...
                      >
                        {item.score}
                        <span style={S.outOf}>/{item.out_of}</span>
                      </span>
                    </div>
                    <div style={S.barBg}>
                      <div
                    ...
                    <p style={S.breakdownReason}>{item.reason}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div style={S.snapshotGrid}>
              {[
                { label: "Level", value: analysisResult.candidate.level },
                {
                  label: "Ready to Apply",
                  value: analysisResult.candidate.ready_to_apply
                    ? "Yes ✓"
                    : "Not Yet",
                },
                {
                ...
                { label: "Market Demand", value: analysisResult.market.demand },
              ].map((item) => (
            ...
            <div style={S.assetRow}>
              ...
              <div style={{ ...S.assetCard, borderColor: "#3a1515" }}>
                <p style={{ ...S.assetLabel, color: "#fca5a5" }}>
                  🚧 Biggest Blocker
                </p>
                <p style={S.assetText}>
            ...
            {analysisResult.skills.ats_keywords_missing?.length > 0 && (
              <>
                <p style={S.sectionLabel}>ATS Keywords to Add</p>
                <div style={S.tagRow}>
            ...
            >
              <div style={S.atsFilterTop}>
                <span
                  style={{
                    ...S.atsBadge,
                    background: analysisResult.ats_filter.will_pass
                      ? "#0d1f14"
                      : "#1e0f0f",
                    color: analysisResult.ats_filter.will_pass
                      ? "#86efac"
                      : "#fca5a5",
                  }}
                >
              ...
              {analysisResult.ats_filter.format_issues.map((issue, i) => (
                <div key={i} style={S.issueRow}>
                  <p style={S.issueText}>⚠ {issue.issue}</p>
              ...
              <div key={i} style={S.fixCard}>
                <div style={S.fixHead}>
                  <span
                    style={{
                      ...S.fixPriority,
                      color: fix.priority === "High" ? "#fca5a5" : "#fde68a",
                      borderColor:
                        fix.priority === "High" ? "#7f1d1d" : "#78350f",
                      background:
                        fix.priority === "High" ? "#3a0f0f" : "#1e1a0f",
                    }}
                  >
                    {fix.priority}
                  </span>
                  <span style={S.fixSection}>{fix.section}</span>
                </div>
            ...
            {analysisResult.action_plan.map((a, i) => (
              <div key={i} style={S.actionRow}>
                <div style={S.actionLeft}>
                  <span style={S.actionTimeline}>{a.timeline}</span>
                  <span
                    style={{
                      ...S.actionImpact,
            ...
            <div style={S.verdictCard}>
              ...
              <p style={S.verdictText}>{analysisResult.final_verdict}</p>
            </div>

            {/* ── Motivation ── */}
            {analysisResult.motivation && (
              <div style={S.motivationCard}>
                <p style={S.motivationText}>"{analysisResult.motivation}"</p>
              </div>
            )}
          </div>
        )}
...
const S = {
  ...
  actionImpact: { fontSize: "10.5px", fontWeight: "600", textAlign: "center" },
  actionText: {
    margin: 0,
    fontSize: "13px",
    color: "#94a3b8",
