...
function Field({ label, error, touched, children }) {
    return (
        <div className='mb-3 sm:mb-4'>
            <label className='block text-sm font-medium text-[#aaa] mb-1 sm:mb-1.5'>
                {label}
            </label>
            {children}
    ...
    return (
        <div className='w-full'>
            ...
            <Field
                ...
                touched={activeErrorField === 'fullName'}
            >
                <div className='relative'>
                    <User size={15} className='absolute left-3.5 top-1/2 -translate-y-1/2 text-[#555]'/>
                    <input type='text' value={values.fullName} onChange={set('fullName')} placeholder='Negan Smith' className={`w-full pl-10 pr-4 py-2 sm:py-2.5 rounded-xl bg-[#111] border text-sm text-white placeholder-[#444] focus:outline-none transition-all duration-200 ${activeErrorField === 'fullName' ? 'border-[#ef4444] focus:border-[#ef4444]' : 'border-[#2a2a2a] hover:border-[#444242] focus:border-[#D9A919]'}`}/>
                </div>
            </Field>
            ...
            <Field
                ...
                touched={activeErrorField === 'email'}
            >
                <div className='relative'>
                    <Mail size={15} className='absolute left-3.5 top-1/2 -translate-y-1/2 text-[#555]'/>
                    <input type='email' value={values.email} onChange={set('email')} placeholder='negan.smith@example.com' className={`w-full pl-10 pr-4 py-2 sm:py-2.5 rounded-xl bg-[#111] border text-sm text-white placeholder-[#444] focus:outline-none transition-all duration-200 ${activeErrorField === 'email' ? 'border-[#ef4444] focus:border-[#ef4444]' : 'border-[#2a2a2a] hover:border-[#444242] focus:border-[#D9A919]'}`}/>
                </div>
            </Field>

            <button onClick={handleContinue} disabled={!isValid || loading} className='w-full mt-2 py-2 sm:py-2.5 rounded-xl bg-[#D9A919] text-black font-semibold text-sm hover:shadow-[0_0_20px_rgba(217,169,25,0.35)] hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-none cursor-pointer'>
                {loading ? (
            ...
            <p className='mt-4 sm:mt-5 text-center text-xs text-[#444] leading-relaxed'>
                ...
                <Link
                to='/terms'
                className='text-[#888] hover:text-[#D9A919] transition-colors duration-200'
                >
                Terms
                </Link>{' '}
                and{' '}
...
function StepOtp({ email, onNext, onBack, active }) {
    ...
    const handleResend = async () => {
        try {
            ...
            setActiveError(
                error.response?.data?.error || 'Failed to resend. Please try again.',
            );
        } finally {
    ...
    return (
        <div className='w-full'>
                ...
                {otp.map((digit, i) => (
                    <input
                        ...
                        ${
                            ...
                            : digit
                                ? 'border-[#D9A919] text-white'
                                : 'border-[#2a2a2a] text-white focus:border-[#D9A919]'
                        }`}
                    />
                ))}
            </div>
            ...
            <p className='mt-3 text-center text-xs text-[#444]'>
                Wrong email?{' '}
                <button onClick={onBack} className='text-[#888] hover:text-[#D9A919] transition-colors duration-200 cursor-pointer'>Go back and change it</button>
            </p>
        </div>
    );
}
        ...
        <div className='w-full'>
            <div className='text-center mb-6 sm:mb-8'>
                ...
                <p className='mt-2 text-sm text-[#8a8686]'>
                    Almost there,{' '}
                    <span className='text-[#D9A919] font-medium'>
                        {name.split(' ')[0]}
                    </span>
                </p>
            </div>
    ...
    return (
        <div className='relative min-h-screen w-full bg-[#0A0A0A] text-white overflow-hidden flex items-center justify-center'>
            ...
            {step === 0 ? (
                ...
                <button onClick={() => goTo(step - 1)} className='group absolute top-6 left-6 flex items-center gap-1.5 text-sm md:text-base text-[#8a8686] hover:text-[#D9A919] transition-colors duration-200 z-10 cursor-pointer'>
                    <ArrowLeft className='duration-300 group-hover:-translate-x-0.5 transition-transform' size={15} strokeWidth={2} />
                    Back
                </button>
            )}
            <div className='relative z-10 w-full max-w-sm mx-4 py-10 sm:py-12'>
                ...
                <div style={{ position: 'relative', minHeight: step === 0 ? undefined : STEP_HEIGHTS[step], transition: 'min-height 0.35s ease'}}>
                    <div style={{ opacity: step === 0 ? 1 : 0, transform: step === 0 ? 'translateX(0px)' : 'translateX(-32px)', transition: 'opacity 0.35s ease, transform 0.35s ease', pointerEvents: step === 0 ? 'auto' : 'none', visibility: step === 0 ? 'visible' : 'hidden' }}>
